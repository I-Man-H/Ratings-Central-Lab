...
function HistoryMode({ events, label, onFile, over, setOver, onClear, pid, setPid }) {
  ...
  const head = (
    <>
      ...
      <DropZone onFile={onFile} over={over} setOver={setOver} compact={events.length > 0}
                title={events.length ? "Load a different rating history" : "Load a rating history"}
                hint="The PlayerHistory CSV, one row per event." />
    </>
  );
  if (!events.length) return head;
  if (!a) return <>{head}<div className="panel empty">Not enough events in this period.</div></>;
      ...
      <div className="wrap body">
        ...
        {tab === "history" ? (
          <HistoryMode
            ...
            over={over}
            setOver={setOver}
            onClear={() => { setEvents([]); setLabel(""); say("Cleared."); }}
          />
        ) : (
          <H2HMode
            matches={matches}
